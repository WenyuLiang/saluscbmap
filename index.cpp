...
void Index::Construct(uint32_t num_sequences, const SequenceBatch &reference) {
  ...
  seeds.reserve(reference.GetNumSequences() *(reflen - 2 * offset_ - kmer_size_ + 1));
  std::cerr << "Collecting seeds.\n";

  SeedGenerator seed_generator(offset_, kmer_size_, length_);
  for (uint32_t sequence_index = 0; sequence_index < num_sequences;
       ++sequence_index) {
    // std::cout << "sequence_index: " << sequence_index << std::endl;
    seed_generator.GenerateSeeds(reference, sequence_index,
                                           seeds);
  }
  ...
  uint64_t previous_lookup_hash =
      GenerateHashInLookupTable(seeds[0].GetHash());
  ...
  for (size_t mi = 0; mi <= num_seeds; ++mi) {
    ...
    // std::cout << "current_lookup_hash: " << (uint32_t)(seeds[mi].GetHit()>>32) << std::endl;
    ...
    //kh_get_val(lookup_table_, khash_iterator) = seeds[mi].GetHit();
      ...
      if (num_previous_seed_occurrences == 1) {
        ...
        //std::cout << "value1: " << (uint32_t)(kh_value(lookup_table_, khash_iterator)>>32) << std::endl;
        ...
        //std::cout << "value2: " << (uint32_t)(kh_value(lookup_table_, khash_iterator)>>32) << std::endl;
      }
...
//         //printf("key = %d, value = %d\n", kh_key(lookup_table_, k), kh_val(lookup_table_, k));
//         std::cout << "value3: " << (uint32_t)(kh_value(lookup_table_, k)>>32) << std::endl;
//     }
// }
}
...
int Index::GenerateCandidatePositions(MappingMetadata &mapping_metadata, SequenceBatch &ref, SequenceBatch &read, uint32_t &i) const {
  ...
  // std::cout << "ref count: " << mapping_metadata.reference_hit_count_.size() << std::endl;
  // std::cout << "read seq: " << read.GetSequenceAt(i) << std::endl;
  // for (auto const &pair: mapping_metadata.reference_hit_count_){
  //     // std::cout << "reference_id: " << ref.GetSequenceAt(pair.first) << " count: " << pair.second << "\n" << std::endl;
...
uint64_t Index::GenerateCandidatePositionFromHits(MappingMetadata &mapping_metadata, uint64_t reference_hit,
